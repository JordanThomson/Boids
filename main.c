...
uint8_t halfSecondDelay = 0;
uint8_t lastButtonPressTime = 0;
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (LL_GPIO_IsInputPinSet(Button0_GPIO_Port, Button0_Pin))
	  {
		...
		lastButtonPressTime = 0;
		  ...
		  {
			  halfSecondDelay = !halfSecondDelay;
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t binVal = read_from_address(patternIndex);

	//If SPI data incorrect
	if (binVal != patterns[patternIndex]){
		binVal = 0b00000001;
	}
